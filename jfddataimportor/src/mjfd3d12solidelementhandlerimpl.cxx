...
bool MJfd3DSolidElementHandlerImpl::initialize(MDataModel& model, bool isRepeated)
{
	...
	ok = _data->_hexaManager.open(_data->_elementPath, "HexaBrick12Element");                                                                           //!!!!!!
	if (!ok)
	{
		QString errorMessage = "can't open HexaBrick12Element DataManager "
				"   in MJfd3DSolidElementHandlerImpl::initialize() ";
		qDebug() << errorMessage;
		_data->_monitor.setMessage(errorMessage);
		return false;
...
bool MJfd3DSolidElementHandlerImpl::handleEntry(QTextStream* stream, QTextStream* logStream, const QVector<QVariant>& value)
{
	...
	QString dataType1 = "org.sipesc.fems.data.hexabrick12elementdata";
	...
	for (int i = 0; i < nMat; i++)
	{
		...
		int matId = matCount + line.mid(0,5).toInt();// |0~4|   :材料号                                        //将组内的每一种材料都在整体情况下编号，存入密度、杨氏模量和泊松比，所有这些形成材料数据
		double DENS = line.mid(5,10).toDouble();//|5~14|:该材料密度                                    //当两个单元材料号相同，能否通过if语句跳过下面的数据读取？？
...
//		int eleId1 = eleCount + 2*line.mid(0,5).toInt()-1;// |0~4|   :单元号                                      //这里是把一个单元分成两个单元来处理，
//		int eleId2 = eleCount + 2*line.mid(0,5).toInt();// |0~4|   :单元号
		int  ematId = matCount + line.mid(20,5).toInt();// |20~24|:材料号                                   //这里的材料号，是指该单元的材料号，通过这个材料号，可以从材料数据文件中调取对应的材料数据
...
//		eleData2.setPropertyId(ematId);
		...
		eleData.setNodeId(4, node5);
		eleData.setNodeId(5, node6);
		eleData.setNodeId(6, node7);
		eleData.setNodeId(7, node8);
		eleData.setNodeId(8, node9);
		eleData.setNodeId(9, node10);
		eleData.setNodeId(10, node11);
		eleData.setNodeId(11, node12);
...
//		eleData2.setNodeId(7, node8);
		...
		_data->_hexaManager.appendData(eleData);                                                       //两个单元的形式，改为一个单元的形式，节点号和卡片数据顺序的对应需要弄清楚，下边的代码，没改
